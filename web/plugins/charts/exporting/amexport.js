MorrishCharts.MorrishExport = MorrishCharts.Class({
	...
	init: function(){
		...
		if (MorrishCharts.isIE && MorrishCharts.IEversion < 10) {
		...
		if (cfg) {
			cfg.menuItemOutput = MorrishCharts.extend(_this.cfg.menuItemOutput, cfg.menuItemOutput || {});
			cfg.menuItemStyle = MorrishCharts.extend(_this.cfg.menuItemStyle, cfg.menuItemStyle || {});
			_this.cfg = MorrishCharts.extend(_this.cfg, cfg);
		}

		// Add reference to chart
		_this.chart.MorrishExport = _this;
	...
	*/
	log: function() {
		console.log('MorrishExport: ', arguments);
	},
	...
	*/
	setup: function() {
		...
		if (!MorrishCharts.isIE || (MorrishCharts.isIE && MorrishCharts.IEversion > 9)) {
	...
	*/
	output: function(cfg, externalCallback) {
		var _this = this;
		cfg = MorrishCharts.extend(MorrishCharts.extend({}, _this.cfg.menuItemOutput), cfg || {});
	...
	*/
	generateOutput: function(cfg, callback) {
		...
		for (var i = 0; i < svgs.length; i++) {
			...
			tmp           = MorrishCharts.extend({}, offset);
			...
			_this.processing.buffer.push([svgClone, MorrishCharts.extend({}, offset)]);
		...
		canvas.id		= MorrishCharts.getUniqueId();
		...
		function drawItWhenItsLoaded() {
			...
			// LOOPING LUI
				...
				if (cfg.render == 'browser') {
					img		= new Image();
					img.id	= MorrishCharts.getUniqueId();
		...
		function createList(items) {
			...
			for (var i = 0; i < items.length; i++) {
				var li = document.createElement('li'),
					...
					item = items[i],
					children = null,
					itemStyle = MorrishCharts.extend(MorrishCharts.extend({}, _this.cfg.menuItemStyle), items[i]);

				// MERGE CFG
				item = MorrishCharts.extend(MorrishCharts.extend({}, _this.cfg.menuItemOutput), item);
				...
				MorrishCharts.extend(a.style, itemStyle);
